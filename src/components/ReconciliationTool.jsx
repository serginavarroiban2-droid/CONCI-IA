...
console.log("Supabase Client Status:", supabase ? "Conectat" : "No detectat (revisa el .env)");
...
export default function ReconciliationTool() {
  ...
  const COL_FAC_NIF = "NIF PROVEEDOR";
  const COL_FAC_TOTAL = "TOTAL FACTURA";
  const COL_FAC_URL = "URL FACTURA";
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!supabase) {
        console.warn('Supabase no configurat. Carregant mode offline.');
        setLoading(false);
        return;
      }
      setLoading(true);
      try {
        ...
        console.error("Detall error Supabase:", error);
        alert('Error carregant dades: ' + error.message + '\n\nHas executat el SQL a Supabase?');
      }
  ...
  // Funció per generar PDF unificat amb tots els documents
  const generateMergedPDF = async () => {
    const origs = filteredInvoices.filter(i => safeGet(i, COL_FAC_URL));
    if (origs.length === 0) { alert('No hi ha factures amb documents originals per imprimir en la selecció actual.'); return; }

    setLoading(true);
    setLoadingStatus(`Generant PDF unificat de ${origs.length} documents...`);
    ...
    try {
      ...
      for (let i = 0; i < origs.length; i++) {
        const inv = origs[i];
        setLoadingStatus(`Processant document ${i + 1} de ${origs.length}...`);

        const url = safeGet(inv, COL_FAC_URL);
        ...
        try {
          ...
          if (isPdf) {
            ...
            // Add Metadata text on top
            const prov = safeGet(inv, COL_FAC_PROV);
            const total = safeGet(inv, COL_FAC_TOTAL);
            const data = safeGet(inv, COL_FAC_DATA);

            page.drawText(`${data} - ${prov} - ${total}€`, {
              x: 30,
              y: height - 20,
              size: 10,
              color: rgb(0, 0, 0),
          ...
          console.error('Error processant document:', inv, err);
          // Optionally add an error page
          const page = mergedPdf.addPage();
          page.drawText(`Error carregant document per: ${safeGet(inv, COL_FAC_PROV)}`, { x: 50, y: 700, size: 20, color: rgb(1, 0, 0) });
        }
      }
      ...
      link.download = `factures_unificades_${selectedYear}_T${selectedQuarters.join('')}.pdf`;
      link.click();

      setLoadingStatus(`✓ PDF generat correctament!`);
      setTimeout(() => setLoadingStatus(''), 2000);

    } catch (e) {
      console.error(e);
      alert('Error generant PDF unificat: ' + e.message);
    } finally {
      setLoading(false);
    }
  };

  // Funció per imprimir els originals de les factures filtrades
  const printOriginals = () => {
    const origs = filteredInvoices.filter(i => safeGet(i, COL_FAC_URL));
    if (origs.length === 0) { alert('No hi ha factures amb documents originals per imprimir en la selecció actual.'); return; }

    const w = window.open('', '_blank');
    let html = `<html><head><title>Factures Originals</title>
      <style>
        ...
        img { max-width: 100%; max-height: 95vh; display: block; margin: 10px 0; box-shadow: 0 0 5px rgba(0,0,0,0.1); }
        .meta { background: #f1f5f9; padding: 10px; border-radius: 5px; font-weight: bold; font-size: 14px; color: #334155; border-left: 4px solid #3b82f6; }
        .pdf-link { display: inline-block; padding: 10px 20px; background: #ef4444; color: white; text-decoration: none; border-radius: 5px; margin-top: 10px; }
      </style>
    </head><body>
    <h1>Llistat de Factures Originals (${origs.length})</h1>`;

    origs.forEach(inv => {
      const url = safeGet(inv, COL_FAC_URL);
      const isPdf = url.toLowerCase().includes('.pdf');
      const prov = safeGet(inv, COL_FAC_PROV);
      const num = safeGet(inv, COL_FAC_NUM) || safeGet(inv, 'NUM FACTURA');
      const data = safeGet(inv, COL_FAC_DATA);
      const total = safeGet(inv, COL_FAC_TOTAL);

      html += `<div class="page">
        <div class="meta">${data} - ${prov} - Nº: ...${num} - <span style="font-size:1.2em">${total}€</span></div>`;

      if (isPdf) {
        html += `<p>⚠️ Aquest document és un PDF i no es pot incrustar directament per imprimir.</p>
                  <a href="${url}" target="_blank" class="pdf-link">Obrir PDF Original</a>`;
      } else {
        html += `<img src="${url}" loading="lazy" onerror="this.style.display='none';this.insertAdjacentHTML('afterend', '<p style=color:red>Error carregant imatge.</p>')" />`;
      }
      html += `</div>`;
    });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div id="win-factures" style={{ resize: 'vertical', height: '600px' }} className="bg-white rounded-xl shadow border flex flex-col overflow-auto min-h-[300px]">
          <div className="p-3 bg-slate-800 text-white flex justify-between items-center no-print sticky top-0 z-20">
            ...
            <button onClick={generateMergedPDF} className="p-1 hover:bg-slate-700 rounded transition ml-1" title="Imprimir originals UNIFICAT (PDF)"><FileText size={16} /></button>
          </div>
